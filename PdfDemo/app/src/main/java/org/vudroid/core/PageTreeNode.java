...
class PageTreeNode {
    ...
    public void setPreBitmap(Bitmap bitmap) {
        ...
        // 计算宽高缩放率
        float scaleWidth = ((float) 275) / width;
        float scaleHeight = ((float) 210) / height;
        // 缩放图片动作
        matrix.postScale(scaleWidth, scaleHeight);
        mPreBitmap = Bitmap.createBitmap(bitmap, 0, 0, (int) width, (int) height, matrix, true);
    }
...
//                            new PageTreeNode(documentView, new RectF(0, 0, 1.0f, 1.0f), page, newThreshold, this)
                            //两次绘制完
                            new PageTreeNode(documentView, new RectF(0f, 0f, 1f, 0.5f), page, newThreshold, this),
                            new PageTreeNode(documentView, new RectF(0f, 0.5f, 1f, 1f), page, newThreshold, this)
                    };
        }
        ...
        documentView.decodeService.decodePage(this, page.index, new DecodeService.DecodeCallback() {
            public void decodeComplete(final Bitmap bitmap) {
                documentView.post(new Runnable() {
                    public void run() {
                        setBitmap(bitmap);
                        //if(mPreBitmap != null)setPreBitmap(bitmap);
    ...
    private void setBitmap(Bitmap bitmap) {
        ...
        if(bitmap != null){
            //documentView.notifyAdapterChange();
        }
    }
